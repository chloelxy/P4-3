...
# Top 10 popular stocks: Apple, Microsoft, Google, Amazon, Tesla, Nvidia, Meta, Netflix, JPMorgan, Visa
...
with tab2:
    ...
    # ---- Chart (same as yours, but add two labels) ----
    fig2 = go.Figure([
        go.Candlestick(
            x=enriched.index,
    ...
    for _, grp in runs.groupby(run_ids):
        ...
        x0, x1 = grp.index.min(), grp.index.max()  # no +1 day (looks cleaner)
        fig2.add_vrect(
            x0=x0, x1=x1,
    ...
    if missing:
        ...
        st.dataframe(enriched[show_cols].tail(20)) #show 20 table rows
